  ...
  return (
    <Enveloppe>
      {(largeur) => {
        ...
        if (!tous.length) return <p className="vide">Selectionner au moins un grimpeur.</p>
        ...
        return (
          <>
            <svg width={largeur} height={hauteur} role="img" aria-label="Progression au fil du temps">
              <Cadre
                largeur={largeur}
                droite={droite}
                hauteur={hauteur}
                      ...
                      <circle
                        ...
                        onMouseMove={(e) =>
                          montrer(e, {
                            ...
                            lignes: [
                              ['Date', dateCourte(p.t)],
                              ['Niveau', `${formatY(p.v)} (${nombre(p.v, 1)})`],
                            ],
                          })
                        }
                        onMouseLeave={cacher}
...
export function MiniCourbe({ valeurs, hauteur = 54 }: { valeurs: number[]; hauteur?: number }) {
  return (
    <Enveloppe>
      {(largeur) => {
        if (valeurs.length < 2) return <p className="discret" style={{ fontSize: 12 }}>Une seule passe : rien a tracer.</p>
        ...
        return (
          <svg width={largeur} height={hauteur} role="img" aria-label="Deplacement moyen par passe">
